...
%                 calib_phase(:,3)=sanitize_phase(uncalib_phase(:,3)');
        ...
        ad_ch_csi_col=[];
        for i=1:30
            new_col=[]; %6 Phase Diff (1 for each antenna pair for each frequency
            for t=1:size(TX,2)
                for r=1:size(RX,2)
                    temp1=csi_col_ser{t,RX{r}(1)}(i,:);    
                    temp2=csi_col_ser{t,RX{r}(2)}(i,:);
                    PhDiff = phdiffmeasure(temp1,temp2);
                    PhDiff = PhDiff*180/pi;
                    if (isnan(PhDiff))
                        PhDiff=0;%mean(phdiff_12);
                    end
                    new_col=[new_col;abs(PhDiff)];
                end
            end
            mu=nanmean(new_col(:));
            ...
            mnval=min(new_col(:));
            new_col=new_col-mu;
            new_col=new_col./(mxval-mnval);
            ad_ch_csi_col=[ad_ch_csi_col new_col];
        end
...
%                 hold off;  
                ...
                plot(1:30,ad_ch_csi_col(obs,:), 'color',rand(1,3), 'LineWidth', 2)
                %ylim([-0.6 0.6])
                hold off;
                obs=obs+1;
            end
...
%         final_op_mat(:,2)=cellstr(sprintf('%d %d',xx,yy));
        row2=1;
        for i=1:min(obs2write,obs)
            new_row=[];
            discard=0;
            new_row=[num2cell(obs_times(row2)) cellstr(sprintf('%d %d',xx,yy)) num2cell(ad_ch_csi_col(i,:))];
